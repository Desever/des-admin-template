<template>
	<div class="public-page">
		<el-page-header @back="goList" :content="'移除权限-'+activeItem.name"></el-page-header>
...
export default {
	...
	created(){
		...
		this.$store.dispatch("role/getIsAddList",{
			...
			this.$refs["addForm"].validate((valid) => {
				if (valid) {
					this.$confirm(`确认修改改角色权限?`, '提示', {
						...
						this.$store.dispatch("role/permissionRevoke",this.addForm).then((res)=>{
							this.$store.dispatch("role/changeAddloading",false);
							if(res.state==1){
								this.$notify({
									title: '提示',
